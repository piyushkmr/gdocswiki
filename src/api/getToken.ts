...
const getTokenFromRefresh = (refreshToken: string): Promise<TokenResponse> => {
  ...
  if (refreshToken) {
    return http.post('https://oauth2.googleapis.com/token', {
      ...
      console.log({ refreshResponse: resp.data });
      return {
...
export const getTokenUsingCode = (code: string, refreshToken?: string): Promise<TokenResponse> => {
  ...
  if (code) {
    return http.post('https://oauth2.googleapis.com/token', {
      ...
      client_id: CLIENT_ID,
      client_secret: CLIENT_SECRET,
      scope: ['https://www.googleapis.com/auth/drive', 'https://www.googleapis.com/auth/userinfo'],
